...
- バッジ表示（check-pro/route.js）:
  - フルプラン契約者 → `フル${契約サイト数}`（例: `フル1`, `フル5`, `フル15`）
  - 戦略診断プラン契約者（1サイト） → `診断`
  - 戦略診断プラン契約者（10/100サイト） → `診断 ${残り回数}/${契約数}`（例: `診断 99/100`）
    - 残り = site_limit - sitesテーブルに登録済みの件数
  - pro_users直接登録 → `無制限`

### 戦略診断プラン（有効期限1年・チャットなし・ワンショット利用・履歴保存なし）
...
- 次回更新日は user_plans.expires_at に保存、check-pro APIで取得しヘッダーに表示
  - フルプラン: 「次回更新: 2025/5/17」
  - 戦略診断プラン: 「有効期限: 2026/4/17」
...
- **プラン名再設計**: スポット→**戦略診断プラン**、サブスク→**戦略診断・策定・アクションプラン**（略称：フルプラン）
- **「立案」→「策定」統一**: タブ名「戦略立案」→「戦略策定」、説明文も全て更新
- **戦略診断プラン=チャットなしワンショット化**: AnalysisChatPanelをチャット権限ある人だけに表示、CTAバナー追加
- **トライアルチャット廃止**: 新規ユーザーへの`is_trial=true`チケット付与を削除（auth/[...nextauth]/route.js, admin/reset-trial/route.js）
- **戦略診断プラン価格改定**: 1サイト¥44,000→¥22,000、10サイト¥110,000、100サイト¥770,000
- **Stripe Price ID更新**: 戦略診断プラン3つ + フルプラン12個すべて新価格で作成済み
...
- [ ] 戦略診断プラン購入フローが新Price IDで動作するか（テストモード）
- [ ] フルプラン購入フローが動作するか
- [ ] 50%OFFクーポンが自動適用されるか
- [ ] PROユーザー（権さん）でフル機能が使えるか
- [ ] トライアルユーザーで戦略診断プラン体験ができるか（チャット非表示確認）

### セミナー後タスク（優先度高）
- **戦略診断結果のメール送信** — 分析完了時にPDFリンクまたはHTML本文をResendで送信。FROM: info@senryaku.ai。テンプレートにフルプラン誘導CTA入れる
- **ダッシュボードの戦略診断ユーザー専用UI** — 現在は全ユーザーに同じサイトリストを表示している。戦略診断プランユーザー向けに「残り◯サイト」のシンプル表示に切り替える
- **チャット履歴・戦略確定履歴のDB保存** — 現在localStorageのみ。ブラウザ変更・キャッシュクリアで消失する。sitesテーブルのchat_historyカラムは存在するので、定期的にDBに同期する仕組みが必要
- **戦略診断プラン＋フルプラン両方所持時の扱い整備** — 2026/04/18時点で暫定処置として「フルプラン優先表示」（check-pro/route.js の ORDER BY で plan_type='support' を優先）で対応。正しい対応は以下いずれか:
  1. フルプラン購入時、既存の戦略診断プランを自動キャンセル＆日割り返金（Stripe側も）
  2. 戦略診断プランユーザーがフルプラン購入ボタンを押した時点で「アップグレード確認」モーダル表示
  3. 両方購入は原則禁止（既に片方を持っていればもう一方のボタンを無効化）
  - 現状の暫定処置: 診断プランはactiveのままDBに温存されるが、バッジ・機能はフルプラン扱い。ユーザーが損することはないが、診断のサイト枠が塩漬けになる。
...
- **戦略診断プランのバッジ表示統一** — 現状「診断（1サイト）」「診断 99/100（10/100サイト）」の2パターン。フルプランは常に「フル1」「フル5」と数字付き。1サイトプランも「診断 1/1 → 診断 0/1」のように残り/総枠表記に統一すると、使用済みかどうかが一目でわかりUX向上（2026/04/18議論）。
