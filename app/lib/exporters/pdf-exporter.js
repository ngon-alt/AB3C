...
function renderCompanyHtml(s) {
  // 権さん 2026-05-18: PPT と同じ見た目を実現するため、HTML を block flow に変更。
  // - 旧: flex:3/2/1.5 + overflow:hidden で固定比率配分（コンテンツ短時に大きな余白）
  // - 新: 各セクションを自然な block で配置し、margin-bottom で共通 gap を確保
  //   → セクション h がコンテンツに自動フィットして「body末尾 → 次ラベル」の gap が揃う
  // PPT に合わせて見出しを強く（16pt → 32px sans bold ink）、本文を二回り小さく（12pt → 24px）
  return `
    <div style="position:absolute;inset:0;">
      ${pageHeaderHtml("自社（Company）", "1a1a14", "PART 1  ─  COMPANY")}
      <div style="padding:30px 100px 60px;">
        <!-- 強み -->
        <div style="margin-bottom:40px;">
          <div style="font-size:32px;font-weight:700;color:#1a1a14;margin-bottom:14px;">強み</div>
          <div style="font-size:24px;color:#1a1a14;line-height:1.5;">
            ${s.strength.length ? lines(s.strength, (t) => `<div style="margin-bottom:4px;">・${esc(t)}</div>`) : "—"}
          </div>
        </div>
        <!-- 仕組み -->
        <div style="margin-bottom:40px;">
          <div style="font-size:32px;font-weight:700;color:#1a1a14;margin-bottom:14px;">仕組み</div>
          <div style="font-size:24px;color:#1a1a14;line-height:1.5;">${esc(s.structure || "—")}</div>
        </div>
        <!-- 価値観 -->
        <div>
          <div style="font-size:32px;font-weight:700;color:#1a1a14;margin-bottom:14px;">価値観</div>
          <div style="font-size:24px;color:#1a1a14;line-height:1.5;">${esc(s.passion || "—")}</div>
        </div>
      </div>
    </div>
  `;
