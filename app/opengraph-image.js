...
export default async function Image() {
  try {
    ...
    // タイトル用（900ウェイト）とキャプション用（400ウェイト）を並列取得
    ...
    const fonts = [];
    if (titleFont) fonts.push({ name: "NotoSansJP", data: titleFont, weight: 900, style: "normal" });
    if (bodyFont) fonts.push({ name: "NotoSansJP", data: bodyFont, weight: 400, style: "normal" });
    ...
    return new ImageResponse(
      (
        ...
        >
          ...
          >
            <span style={{ fontSize: 180, lineHeight: 1, fontFamily: "NotoSansJP", fontWeight: 900 }}>戦略指南</span>
          ...
          <div
            style={{
              fontSize: 36,
              color: "#1a1a14",
              fontFamily: bodyFont ? "NotoSansJP" : undefined,
              fontWeight: 400,
              letterSpacing: "0.05em",
            }}
          >
