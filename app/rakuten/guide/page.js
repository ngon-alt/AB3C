...
export default function RakutenGuidePage() {
  return (
    <main style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px 80px", fontFamily: "var(--font-body)", color: "#000" }}>
      ...
      <ol style={{ fontSize: 18, lineHeight: 2.0 }}>
        <li><strong>ターゲットの悩み</strong> —— 「こんなお悩みありませんか」と、お客様の課題への共感から入る</li>
        <li><strong>ベネフィット</strong> —— 開いた瞬間に「何の商品で、何が得られるか」が分かる（3秒が勝負）</li>
        <li><strong>他社との違い</strong> —— なぜ他ではなくこれなのかを示す。比較表があると構図が一目で伝わる</li>
        <li><strong>証拠（信頼コンテンツ）</strong> —— レビューの量と質、売り手が専門家として語るおすすめの理由、第三者の証明（認証・受賞・メディア掲載）、開発ストーリー。レビューがまだ少ない店は、他の3つで客観的な信頼を補う</li>
        <li><strong>使用シーン</strong> —— 論理ではなく情緒で。「そのシーンなら、こう感じるな」を想像してもらう</li>
        <li><strong>不安解消</strong> —— よくある質問・保証・スペック・配送納期・送料がページの中で分かること。ページを離れないと分からない情報は不安になる</li>
        <li><strong>今買う理由</strong> —— 最後のひと押し。今このページで買う理由を示す</li>
      </ol>
      <p style={pStyle}>
        実はこの順番は、お客様（の悩み）→ ベネフィット → アドバンテージ → その根拠、という戦略の物語の順番そのものです。商品ページを直すことは、戦略を語り直すことと同じなのです。
      </p>
