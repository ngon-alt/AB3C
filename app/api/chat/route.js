...
具体的な改善案を優先度順に提示してください。`,
    lp: `\n【初回アドバイス】確定したAB3C分析結果（戦略メッセージ・Benefit・Advantage・3C）を、商品LP（ECの商品ページ・ランディングページ）の「7つの観点」の物語順に流し込んだLP構成案を提案してください。

LPは上から下へ、次の7つの観点の順で語ります。この並びはAB3Cの物語順と一致しています:
${lpViewpointsText}

出力の形式:
- ①〜⑦の観点ごとに「このブロックに何を書くか」の構成案（設計図）を、確定戦略から導いて具体的に提示してください。文章の完成原稿ではなく、各ブロックの狙い・載せる要素・見出しの方向性が分かるレベルで書いてください
- 各観点の構成案には「いまBenefitを語るブロックです」「ここはAdvantageの根拠にあたります」のように、AB3Cとの対応を自然に添えてください
- 各観点の最後に、経営者ご本人にしか分からない事実（開発の経緯・製法・お客様の声・サービスの実態など）を引き出すための確認質問を1〜2個添えてください。AIだけで書ける一般論を埋め込まず、本人の事実で埋めるべき箇所を質問として明示することが重要です
最後に「気になる観点の番号を教えていただければ、その観点から一緒に磨いていきましょう」という趣旨の一文を添えてください。`,
...
${lpViewpointsText}
あなたの役割は書く代行ではなく、ガイド型インタビューです。確定戦略から下書き仮説を先に提示し、経営者ご本人にしか分からない事実（開発の経緯・製法・お客様の声・サービスの実態など）を質問で引き出し、合作で原稿に仕上げてください。本人の事実が入っていない一般論の文章を量産しないでください。「①だけ磨きたい」「④の証拠が弱い」のような観点単位の相談にも、その観点に絞って応じてください。チャット中は「いまBenefitを書いています」「ここはAdvantageの根拠です」のようにAB3C用語との対応を自然に使って構いません。`,
