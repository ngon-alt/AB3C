...
- ロゴは現行サイトのヘッダーにあるものを参照・踏襲する（外部で別ロゴ・旧ロゴを拾わない。ユーザーがロゴ画像を添付すればそれを優先）。配色は、決まっていなければそのロゴの色から検討する（戦略・ブランドイメージに合う場合のみ採用）。ベージュの多用は避ける（没個性でAI生成っぽく見える）
...
- 会社名・固有名詞・実績数値などのアイデンティティ情報は、分析対象の現行サイトと確定分析だけを根拠にする。外部検索で社名・実績を補完しない（旧社名・旧実績を拾って誤記するため）。現行サイトと食い違う情報や確証のないものは採用せず「要確認」と明記する
