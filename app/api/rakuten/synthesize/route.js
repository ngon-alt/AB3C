...
# 出力（JSONのみ・すべて店長に直接見せる日本語で書く）
{
  ...
  "story2": {
    ...
    "sixAxes": [
      { "axis": "機能", "own": "自社の状況（強み弱みを率直に・40字以内）", "competitorBest": "競合で最も強い状況（40字以内）", "verdict": "突出している | 勝っている | 並んでいる | 負けている | 比較材料なし" }
      // デザイン・パッケージング・購入前サービス・購入中サービス・購入後サービス・価格・納期/配送 も同様に
      // 「突出している」は競合を大きく引き離して勝っている場合のみ
    ],
  ...
  "story3": {
    ...
    "focus": {
      "options": [
        { "target": "ターゲット候補（誰）", "benefit": "そのターゲットに対するベネフィット", "reason": "この案の強さ・弱さの評価（レビューの根拠つきで60〜100字）", "chosen": true }
        // 2〜3案。レビューとLPから見えた「選ばれ方」の候補を比較し、最も戦略が強く立つ1案だけ chosen: true にする
      ],
      "ab3c": {
        "customer": "絞ったターゲットの顧客像（具体的に・80字程度）",
        "company": "自社（この商品・店）の本質的な強み",
        "competitor": "競合の戦い方の要約（誰がどの軸で戦っているか）",
        "benefit": "絞ったベネフィット（顧客が得る価値）",
        "advantage": "アドバンテージ（競合ではなくこの店が選ばれる理由）"
      },
    ...
    "stage1": {
      ...
      "topPriorities": [
        { "rank": 1, "axis": "お客様が比較している価値軸（例: 味）", "why": "なぜこれを重視すべきか。レビューでの言及の多さ・熱量・競合との関係を根拠に、80〜120字で具体的に" }
        // 上位3つまで
      ],
      "appealReset": ["訴求ポイントの再設定案（LPの語り方をどう変えるか。focus で絞った戦略に沿って具体的に最大4つ）"],
      "contentPriorities": ["コンテンツ増強の優先順位（最大3つ）"]
    },
    "stage2": {
      "title": "価値そのものを高める（戦略的な改善）",
      "valueDevelopment": [
        { "axis": "機能 | デザイン | パッケージング | 購入前サービス | 購入中サービス | 購入後サービス のいずれか", "proposal": "その軸を強化する商品・サービス開発のアイデア（80〜120字。具体的に）" }
        // 3〜6つ。弱い軸の補強だけでなく、次の問いで発想を広げる:
        // 「全社が手を抜いている軸はどこか」「業界の常識で誰もやっていないことは何か」
        // 「ギフト・保存・開封後など、商品が使われる文脈で誰も応えていない場面はないか」
        // （実例: 精肉業界は『生ものだからパッケージに凝らない』が常識だったが、ギフト文脈では高級パッケージが空白で、そこを取った店が大きく伸びた）
      ],
      "scenarios": [
